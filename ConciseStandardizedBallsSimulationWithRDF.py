...
histogram_sums = np.zeros(50)  # Corrected to have a size of 50

# Set up the figure and axes for the plots
fig, (ax_box, ax_hist) = plt.subplots(1, 2, figsize=(15, 6))
plt.ion()  # Turn on interactive mode for live updates

num_bins = 25  # 50r total range, but now with each bin as 2r
bin_size = 2 * RADIUS  # 0.4

# Initialize the total histogram sums
total_histogram_sums = np.zeros(num_bins)
...
for move in range(MOVES):
    ...
    ax_hist.bar(np.arange(len(histogram_sums)) * bin_size, np.cumsum(histogram_sums), width=bin_size)
    ax_hist.set_xlim(0, 50 * RADIUS)
    ax_hist.set_xticks(np.arange(0, 50 * RADIUS, bin_size))
    ax_hist.set_xticklabels([f'{i * 2}r' for i in range(num_bins)], rotation='vertical')
...
ax_avg_hist.bar(np.arange(len(average_histogram_sums)) * 2 * RADIUS, np.cumsum(average_histogram_sums), width=2 * RADIUS)
ax_avg_hist.set_xlim(0, 50 * RADIUS)
ax_avg_hist.set_xticks(np.arange(0, 50 * RADIUS, 2 * RADIUS))
ax_avg_hist.set_xticklabels([f'{i * 2}r' for i in range(1, 26)], rotation='vertical')
...
# Calculate RDF for the updated bin size
rdf = np.zeros(num_bins)
area_of_bins = np.pi * (np.arange(1, num_bins + 1)**2 * bin_size**2 - np.arange(0, num_bins)**2 * bin_size**2)
density = NUM_BALLS / (BOX_WIDTH * BOX_HEIGHT)
for i in range(num_bins):
    rdf[i] = (average_histogram_sums[i] / area_of_bins[i]) / density

# Plot RDF for the updated bin size
plt.figure()
plt.plot(np.arange(num_bins) * bin_size, rdf)
plt.xlabel('Distance')
plt.ylabel('g(r)')
